...
/* --- LES COMPTES DE MARCHE, D'UN RELEVE A L'AUTRE ---------------------------

   LA MEME DIFFERENCE DE RELEVES, sur un autre perimetre, plus UN chiffre que
   le detenteur declare : ce qu'il a verse sur ces comptes depuis le releve
   d'avant, retraits deduits (`monthly[i].apportsMarche`). Rien d'autre n'est
   lu -- ni achat, ni prix de revient, ni quantite -- parce que rien d'autre
   n'est observe.

   CE QUE LE PERIMETRE FAIT A LUI SEUL. La valeur d'un compte de marche compte
   ses titres ET son cash. Un achat deplace de l'argent du cash vers une ligne
   du meme compte, une vente l'inverse : la valeur du compte n'en bouge pas, et
   aucun des deux n'est un apport. Un virement entre deux comptes du perimetre
   s'annule dans la somme. Un virement depuis un compte courant, lui, entre :
   c'est un apport au portefeuille, sans en etre un au patrimoine -- d'ou un
   champ distinct de `budget.apports`, qui dit ce qui entre dans le patrimoine.

   LA SOMME TIENT PAR CONSTRUCTION. Sur un intervalle declare, l'ecart de
   valeur vaut l'apport plus ce qui ne vient pas de lui ; sur un intervalle non
   declare, tout l'ecart est non attribue. Rien n'est devine : un champ vide
   n'est pas un zero.

   « Hors apports » et non « performance » : ce reste contient les cours, le
   change, les dividendes et les frais, que l'application ne suit pas un par
   un. */

function comptesDeMarche() {
  return ACCOUNTS.filter(a => a.compte ? !!(typeCompte(a.compte.type) || {}).titres
                                        : a.group === 'bourse' && a.role !== 'margin');
}

const valeurMarcheDuReleve = (row, ids = comptesDeMarche().map(a => a.id)) =>
  round2(ids.reduce((s, id) => s + num((row.v || {})[id]), 0));

/* Ce que le releve declare avoir ete verse, ou `null` s'il ne le dit pas. Une
   valeur que la porte des nombres refuserait vaut une absence : elle ne
   s'additionne pas a des apports vrais. */
const apportMarcheDeclare = row => row && estDeclare(row.apportsMarche) && nombreValide(row.apportsMarche, 'signe')
  ? round2(num(row.apportsMarche)) : null;

function intervallesMarche() {
  const lignes = (Store.state.monthly || []).filter(r => !rowIsEmpty(r));
  const comparables = sansDoublonDeCloture(lignes);
  const ids = comptesDeMarche().map(a => a.id);
  const out = [];
  for (let i = 1; i < comparables.length; i++) {
    const a = comparables[i - 1], b = comparables[i];
    const decl = lignes.filter(r => r.date > a.date && r.date <= b.date).map(apportMarcheDeclare);
    const apports = decl.length && decl.every(x => x !== null)
      ? round2(decl.reduce((s, x) => s + x, 0)) : null;
    const avant = valeurMarcheDuReleve(a, ids), apres = valeurMarcheDuReleve(b, ids);
    const variation = round2(apres - avant);
    out.push({ depuis: a.date, jusqua: b.date, mois: moisEntre(a.date, b.date), avant, apres,
               variation, apports, horsApports: apports === null ? null : round2(variation - apports) });
  }
  return out;
}

/* Le bilan d'une suite d'intervalles. `variation` vaut toujours
   `apports + horsApports + nonAttribue`, au centime. */
function bilanMarche(iv) {
  const l = iv || [];
  const declares = l.filter(x => x.apports !== null);
  const somme = (liste, k) => round2(liste.reduce((s, x) => s + x[k], 0));
  return {
    depuis: l.length ? l[0].depuis : null, jusqua: l.length ? l[l.length - 1].jusqua : null,
    mois: l.reduce((s, x) => s + x.mois, 0),
    avant: l.length ? l[0].avant : 0, apres: l.length ? l[l.length - 1].apres : 0,
    variation: somme(l, 'variation'),
    apports: somme(declares, 'apports'),
    horsApports: somme(declares, 'horsApports'),
    nonAttribue: somme(l.filter(x => x.apports === null), 'variation'),
    intervalles: l.length, declares: declares.length,
    avecApport: declares.filter(x => x.apports > 0).length,
  };
}
